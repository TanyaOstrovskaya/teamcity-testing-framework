...
workdir="teamcity_test_infrastructure"
...
docker run -d --name selenoid_container_name -p 4444:4444 -v /var/run/docker.sock:/var/run/docker.sock  -v $(pwd)/config:/etc/selenoid/config aerokube/selenoid:latest-release
...
cd .. && cd .. && cd ..
echo "host=$ip:8111\nsuperUserToken=$superuser_token\nremote=true" > $teamcity_test_directory/src/main/resources/config.properties
